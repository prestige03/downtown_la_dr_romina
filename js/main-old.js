...
function documentReadyInit() {
	...
	if (jQuery().tweet) {
		jQuery('.twitter').tweet({
			...
			template: "<div class=\"tweet_right\">{join}<span class=\"tweet_text\">{tweet_text}</span><span class=\"darklinks\">{time}</span></div>"
		});
	}
		...
		if ( $flickr.length ) {
			if ( ! ( $flickr.hasClass('flickr_loaded') ) ) {
				$flickr.jflickrfeed({
					...
					itemTemplate: '<li><a href="{{image_b}}" data-gal="prettyPhoto[pp_gal]"><img alt="{{title}}" src="{{image_m}}" /></a></li>'
				}, function(data) {
